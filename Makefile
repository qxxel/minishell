...
SOURCES				=	builtins/ft_cd.c				\
						...
						parse_cmds/compute_wildcard.c	\
						parse_cmds/expand_quotes.c		\
						parse_cmds/expand_wildcards.c	\
...
RM					=	rm -rf
MKDIR				=	mkdir -p
VALGRIND			=	valgrind --trace-children=yes --track-fds=yes --show-error-list=yes --suppressions=$(shell pwd)/.valgrind.supp --read-var-info=yes --leak-check=full --show-leak-kinds=all --track-origins=yes
